...
void CreatureObjectImplementation::addSkill(Skill* skill, bool notifyClient) {
	...
	// Project Kamino: do not summon the helper droid when a new player learns
	// one of the six basic novice professions.
	constexpr bool helperDroidProfessionHelpEnabled = false;

	if (helperDroidProfessionHelpEnabled && isPlayerCreature()) {
...
void CreatureObjectImplementation::addSkillMod(const int modType, const String& skillMod, int value, bool notifyClient) {
	...
	if (modType == SkillModManager::TEMPLATE && skillMod == "jedi_force_power_max" && value == 250){
		info("Not adding force max stat from old robes", true);
		return;
	}
	
	if (modType == SkillModManager::TEMPLATE && skillMod == "jedi_force_power_regen" && value == 10){
		info("Not adding force regen stat from old robes", true);
		return;
	}
...
bool CreatureObjectImplementation::isAttackableBy(TangibleObject* object, bool bypassDeadCheck) {
	...
	*/
	...
	// passenger seats should be unattackable
	if (isPassengerSeat())
		return false;
...
/*
	if (creature->isPlayerCreature() && (getPvpStatusBitmask() & CreatureFlag::FRIENDLY))
		return false;

	if (isPlayerCreature() && (creature->getPvpStatusBitmask() & CreatureFlag::FRIENDLY))
		return false;
...
*/
	...
	// All passenger seats should be unattackable
	if (isPassengerSeat()){
		return false;
	}
			...
			if (thisFaction != creatureFaction) {
				if (covertOvert) {
					...
					// TEF vs any non neutral
						...
						return true;
					} else if (thisGcwTef && (creatureFaction == 0 && targetGcwTef)) {
						return true;
					} else {
						return false;
					}
...
bool CreatureObjectImplementation::hasBountyMission() {
	...
	return mission == nullptr ? false : true;
}
...
//to create set items modname would be something like itemsetuber24 and boxes would be named itemsetuber242 and itemsetuber244
//that would mean you gain bonuses for 2 and 4 pieces and the boxes are then granted at 2 and 4 pieces
