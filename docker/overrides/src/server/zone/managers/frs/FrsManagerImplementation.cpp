...
void FrsManagerImplementation::updatePlayerSkills(CreatureObject* player) {
	...
	// Project Kamino FRS bonuses are stored in their own permanent skill-mod
	// group so they stack with AOTC's original rank skills and can be replaced
	// cleanly on promotion, demotion, or council removal.
	int forcePoolBonus = 0;
	...
	auto replaceKaminoMod = [player](const String& modName, int newValue) {
		constexpr int modType = SkillModManager::PERMANENTMOD + 0x0A;
		int oldValue = player->getSkillModOfType(modName, modType);

		if (oldValue != 0)
			player->removeSkillMod(modType, modName, oldValue, true);
		if (newValue != 0)
			player->addSkillMod(modType, modName, newValue, true);
	};

	replaceKaminoMod("jedi_force_power_max", forcePoolBonus);
	replaceKaminoMod("jedi_force_power_regen", playerRank >= 11 ? 50 : 0);
	replaceKaminoMod("saber_skill", playerRank >= 11 ? 50 : 0);

	// Rank 11 grants Force Armor 2. Keep this explicit so characters who
	// already held Council Leader before the skill-table update receive it
	// the next time their FRS skills are synchronized.
	if (playerRank >= 11 && !ghost->hasAbility("forceArmor2"))
		skillManager->addAbility(ghost, "forceArmor2", true);
...
void FrsManagerImplementation::recoverJediItems(CreatureObject* player) {
	...
	if (inventory->transferObject(robeObj, -1, true)) {
		...
		if(robeObj->isWearableObject()){
			WearableObject* wo = cast<WearableObject*>(robeObj.get());
			wo->setSockets(8);
		}
