...
function JediTrials:unlockJediPadawan(pPlayer, dontSendSui)
	...
	if (pInventory == nil or SceneObject(pInventory):isContainerFullRecursive()) then
		...
		TangibleObject(pItem):setSocketCount(6);
	end
...
function JediTrials:completeKnightForTesting(pPlayer, councilType, skipEnclaveTeleport)
	writeScreenPlayData(pPlayer, "KnightTrials", "startedTrials", 1)
	...
	if (skipEnclaveTeleport == true) then
		return
	end
