...
1. The Series 7 combine reward object must inherit the authentic shared
   `combine_reward_deed_republic_gunship` template.
2. Its server Lua must specify `templateType = VEHICLEDEED`, the gunship PCD
...
5. Seven passenger creatures must be registered under the
   `tcg_republic_gunship` seat prefix, and the mobile object must specify
   `passengerCapacity = 7` and `passengerSeatString = "tcg_republic_gunship"`.

The clean Core3 source reference is in `../flurry-gunship-reference`.  It
matches this mapping and is the implementation to port after the client
assets are verified.

## Required client package before activation

The following original client paths must exist in a loaded client TRE.  Do
not activate the Core3 port against substitute AV-21 templates.
