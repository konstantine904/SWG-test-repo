...
bool VehicleObjectImplementation::slotPassenger(CreatureObject* passenger) {
	...
	// The seat creature must be created at the vehicle's world position so the
	// mount controller action passes its range check.  A rider's final local
	// offset is applied only after that creature has been slotted in the vehicle.
	float seatX = x;
	float seatY = y;
	float seatZ = z;

	// The stock Republic Gunship model has no passenger hardpoints.  Its seats
	// therefore need an explicit local transform rather than inheriting another
	// vehicle's rider arrangement.  These seven positions place riders in the
		// troop bay, in paired port/starboard rows behind the cockpit.  Positive
		// local Y is aft on this model; negative values put riders on the nose.
	if (getObjectTemplate()->getFullTemplateString().contains("tcg_republic_gunship")) {
		static const float gunshipSeatPositions[6][3] = {
			{ -1.15f, 3.10f, 0.95f }, { 1.15f, 3.10f, 0.95f },
			{ -1.15f, 4.45f, 0.95f }, { 1.15f, 4.45f, 0.95f },
			{ -1.15f, 5.80f, 0.95f }, { 1.15f, 5.80f, 0.95f }
		};

		if (seatNumber >= 1 && seatNumber <= 6) {
			seatX = gunshipSeatPositions[seatNumber - 1][0];
			seatY = gunshipSeatPositions[seatNumber - 1][1];
			seatZ = gunshipSeatPositions[seatNumber - 1][2];
		}
	}
	...
	seatObject->setPosition(seatX, seatZ, seatY);
	slocker.release();
	...
	Core::getTaskManager()->executeTask([=]() {
		...
		passenger->setPosition(seatX, seatZ, seatY);
		plocker.release();
		...
		// The original Gunship has no passenger hardpoints. Keep passenger travel
		// functional without showing riders at an incorrect exterior position.
		// This uses the normal creature-height delta only; it deliberately avoids
		// a second zone or parent transfer while the mount is being processed.
		auto parentVehicle = seatObject->getParent().get();
		if (parentVehicle != nullptr && parentVehicle->getObjectTemplate()->getFullTemplateString().contains("tcg_republic_gunship")) {
			passenger->setLuaStringData("gunshipPassengerOriginalHeight", String::valueOf(passenger->getHeight()));
			passenger->setHeight(0.01f, true);
		}
	}, "slotPlayerPassenger");
