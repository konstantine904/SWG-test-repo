...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		// Gunship passengers are rendered at a minimal scale while attached because
		// the stock model has no rider hardpoints. Restore the saved height only
		// when this Gunship-specific mount path set it.
		String gunshipPassengerOriginalHeight;
		if (vehicle->isPassengerSeat()) {
			auto parentObject = vehicle->getParent().get();
			if (parentObject != nullptr && parentObject->getObjectTemplate()->getFullTemplateString().contains("tcg_republic_gunship")) {
				gunshipPassengerOriginalHeight = creature->getLuaStringData("gunshipPassengerOriginalHeight");
			}
		}
		...
		for (int i = 1; i < 8; ++i) {
			...
			if (seat != nullptr) {
				...
				if (rider != nullptr) {
					...
					info(true) << "null rider when trying to dismount";
		...
		if (gunshipPassengerOriginalHeight.length() > 0) {
			creature->setHeight(Float::valueOf(gunshipPassengerOriginalHeight), true);
			creature->deleteLuaStringData("gunshipPassengerOriginalHeight");
		}
