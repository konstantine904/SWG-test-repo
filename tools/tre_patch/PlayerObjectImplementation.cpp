...
void PlayerObjectImplementation::initializeAccount() {
	...
	if (account != nullptr && getAdminLevel() != account->getAdminLevel()) {
		CreatureObject* creature = dynamic_cast<CreatureObject*>(parent.get().get());
		if (creature != nullptr)
			getZoneServer()->getPlayerManager()->updatePermissionLevel(creature, account->getAdminLevel());
	}
}
...
void PlayerObjectImplementation::notifyOnline() {
	...
	if (playerCreature->hasSkill("prequel_basic_novice")){
		if (playerCreature->hasSkill("gcw_rank_novice")){
			SkillManager::instance()->surrenderSkill("gcw_rank_novice", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_master", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t1_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t1_02", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t1_03", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t1_04", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t2_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t2_02", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t2_03", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t2_04", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t3_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t3_02", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t3_03", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t3_04", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t4_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t4_02", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t4_03", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("gcw_rank_t4_04", playerCreature, true, true);
		}
		if (playerCreature->hasSkill("combat_brawler_novice")){
			SkillManager::instance()->surrenderSkill("combat_brawler_novice", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_brawler_unarmed_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_brawler_1handmelee_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_brawler_polearm_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_brawler_2handmelee_01", playerCreature, true, true);
		}
		if (playerCreature->hasSkill("science_medic_novice")){
			SkillManager::instance()->surrenderSkill("science_medic_novice", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("science_medic_injury_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("science_medic_ability_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("science_medic_injury_speed_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("science_medic_crafting_01", playerCreature, true, true);
		}
		if (playerCreature->hasSkill("combat_marksman_novice")){
			SkillManager::instance()->surrenderSkill("combat_marksman_novice", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_marksman_rifle_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_marksman_pistol_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_marksman_carbine_01", playerCreature, true, true);
			SkillManager::instance()->surrenderSkill("combat_marksman_support_01", playerCreature, true, true);
		}
	}

	//
	if (!playerCreature->hasSkill("force_title_jedi_novice")){
		SkillManager::instance()->awardSkill("force_title_jedi_novice", playerCreature, true, true, true);
	}
	
	if (playerCreature->hasSkill("prequel_basic_novice") && !playerCreature->hasSkill("force_title_jedi_rank_01")){
		SkillManager::instance()->awardSkill("force_title_jedi_rank_01", playerCreature, true, true, true);
	}else if (!playerCreature->hasSkill("prequel_basic_novice") && playerCreature->hasSkill("force_title_jedi_rank_01")){
		SkillManager::instance()->surrenderSkill("force_title_jedi_rank_01", playerCreature, true, true);
	}
	
	if (playerCreature->hasSkill("prequel_basic_master") && !playerCreature->hasSkill("force_title_jedi_rank_02")){
		SkillManager::instance()->awardSkill("force_title_jedi_rank_02", playerCreature, true, true, true);
	}else if (!playerCreature->hasSkill("prequel_basic_master") && playerCreature->hasSkill("force_title_jedi_rank_02")){
		SkillManager::instance()->surrenderSkill("force_title_jedi_rank_02", playerCreature, true, true);
	}
	
	//Adjust stats to remove mods from old jedi robes
	int robeForceMax = playerCreature->getSkillModOfType("jedi_force_power_max", SkillModManager::TEMPLATE);
	int robeForceRegen = playerCreature->getSkillModOfType("jedi_force_power_regen", SkillModManager::TEMPLATE);
		
	if (robeForceMax == 250){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_max", robeForceMax, true);
	}else if(robeForceMax == -250){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_max", -125, true);//Can't add 250 at once as it will be blocked
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_max", -125, true);//due to block on old robe objects
	}else if(robeForceMax < 0){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_max", robeForceMax, true);//Can't add 250 at once as it will be blocked
	}
	
	if (robeForceRegen == 10){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_regen", robeForceRegen, true);
	}else if(robeForceRegen == -10){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_regen", -5, true);//Can't add 10 at once as it will be blocked
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_regen", -5, true);//due to block on old robe objects
	}else if(robeForceRegen < 0){
		playerCreature->removeSkillMod(SkillModManager::TEMPLATE, "jedi_force_power_regen", robeForceRegen, true);//Can't add 10 at once as it will be blocked
	}

	// Remove set bonus skills if player does not have any pieces of the set.
	if (playerCreature->getSkillMod("itemsetcm235") == 0)
	{
		if(playerCreature->hasSkill("itemsetcm2352")){
			SkillManager::instance()->surrenderSkill("itemsetcm2352", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetcm2353")){
			SkillManager::instance()->surrenderSkill("itemsetcm2353", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetcm2355")){
			SkillManager::instance()->surrenderSkill("itemsetcm2355", playerCreature, true, true);
		}
	}
	if (playerCreature->getSkillMod("itemsetcommando236") == 0)
	{
		if(playerCreature->hasSkill("itemsetcommando2362")){
			SkillManager::instance()->surrenderSkill("itemsetcommando2362", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetcommando2363")){
			SkillManager::instance()->surrenderSkill("itemsetcommando2366", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetcommando2363")){
			SkillManager::instance()->surrenderSkill("itemsetcommando2366", playerCreature, true, true);
		}
	}
	if (playerCreature->getSkillMod("itemsetris236") == 0)
	{
		if(playerCreature->hasSkill("itemsetris2362")){
			SkillManager::instance()->surrenderSkill("itemsetris2362", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetris2363")){
			SkillManager::instance()->surrenderSkill("itemsetris2363", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetris2366")){
			SkillManager::instance()->surrenderSkill("itemsetris2366", playerCreature, true, true);
		}
	}
	if (playerCreature->getSkillMod("itemsetmandoarmour236") == 0)
	{
		if(playerCreature->hasSkill("itemsetmandoarmour2362")){
			SkillManager::instance()->surrenderSkill("itemsetmandoarmour2362", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetmandoarmour2363")){
			SkillManager::instance()->surrenderSkill("itemsetmandoarmour2363", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetmandoarmour2366")){
			SkillManager::instance()->surrenderSkill("itemsetmandoarmour2366", playerCreature, true, true);
		}
	}
	if (playerCreature->getSkillMod("itemsetfactionarmour236") == 0)
	{
		if(playerCreature->hasSkill("itemsetfactionarmour2362")){
			SkillManager::instance()->surrenderSkill("itemsetfactionarmour2362", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetfactionarmour2363")){
			SkillManager::instance()->surrenderSkill("itemsetfactionarmour2363", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetfactionarmour2366")){
			SkillManager::instance()->surrenderSkill("itemsetfactionarmour2366", playerCreature, true, true);
		}
	}
	if (playerCreature->getSkillMod("itemsetbharmour24") == 0)
	{
		if(playerCreature->hasSkill("itemsetbharmour242")){
			SkillManager::instance()->surrenderSkill("itemsetbharmour242", playerCreature, true, true);
		}
		if(playerCreature->hasSkill("itemsetbharmour244")){
			SkillManager::instance()->surrenderSkill("itemsetbharmour244", playerCreature, true, true);
		}
	}
	

	//Set Jedi XP to 0 pre unlock
	ManagedReference<PlayerManager*> playerManager = zoneServer->getPlayerManager();
	if (!playerCreature->hasSkill("force_title_jedi_rank_01")){
		int curExp = (playerCreature->getSlottedObject("ghost").castTo<PlayerObject*>())->getExperience("jedi_general");
		if (curExp < 0){
			playerManager->awardExperience(playerCreature, "jedi_general", curExp*-1, false, 1, false);
		}
	}
	...
	*luaOnPlayerLoggedIn << playerCreature;
	...
	// Update player lot count for account-based lots.
	// maximumLots is given a value in the constructor so there's no way to pull it out here :(
	Reference<PlayerObject*> ghost = playerCreature->getPlayerObject();
	ghost->setMaximumLotCount(ConfigManager::instance()->getInt("Core3.PlayerCreationManager.MaxLotsPerAccount", 40));
}
...
void PlayerObjectImplementation::setMaximumLotCount(int newValue) {
	maximumLots = newValue;
}
...
// Account-lot based implementation
int PlayerObjectImplementation::getLotsRemaining() {
	//Locker locker(asPlayerObject());
	...
	for (int i = 0; i < characters->size(); ++i) {
		...
		if(entry->getGalaxyID() == getZoneServer()->getGalaxyID()) {
			...
			if (targetChar == nullptr || activeSessions >= 1)
				return 0;
			if (targetChar->containsActiveSession(SessionFacadeType::PLACESTRUCTURE))
				activeSessions += 1;
